...
def run_summarizer(session_id, video_id, collect_time, prompt_template):
    try:
        ...
        while True:
            ...
            with data_lock:
                result_dict[session_id] = summary_result
        ...
        print(f"에러 발생: {str(e)}")
        with data_lock:
            result_dict[session_id] = f"에러 발생: {str(e)}"
    finally:
        with status_lock:
...
def summary():
    ...
    return render_template('summary.html', session_id=session_id, collect_time=collect_time)
...
def update_summary():
    session_id = request.json.get('session_id')
    with data_lock:
        summary_result = result_dict.get(session_id, '')
